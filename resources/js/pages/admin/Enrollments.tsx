...
export default function Enrollments({ requests, stats }: Props) {
    ...
    return (
        <AdminLayout>
            ...
            {/* Reject Modal */}
            <Modal show={showRejectModal && !!selectedRequest} onClose={() => { setShowRejectModal(false); setData('rejection_reason', ''); }} title={`Reject Enrollment — ${selectedRequest?.child_first_name} ${selectedRequest?.child_last_name}`}>
                <div className="space-y-4">
                    ...
                    <div className="flex gap-2 justify-end">
                        <button onClick={() => { setShowRejectModal(false); setData('rejection_reason', ''); }} className="px-4 py-2 rounded-xl border border-slate-200 text-slate-600 text-sm font-semibold hover:bg-slate-50 transition-all">Cancel</button>
                        <button onClick={submitRejection} disabled={processing || !data.rejection_reason} className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-red-500 text-white text-sm font-semibold hover:bg-red-600 disabled:opacity-50 transition-all">
                            <X className="w-3.5 h-3.5" />
